...
if (!file_exists($file_path)) {
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx', 'png', 'jpg', 'jpeg', 'doc'])) {
        ...
        if (move_uploaded_file($file, $file_path.'/'.$filename)) {
            $sql = "INSERT INTO tblfiles (`member`, `name`, `dateupload`, `reason`) VALUES ('$owner','$filename', '$dates', '$reason')";
...
<head>
  ...
  <!-- Favicons -->
  <link href="https://liquag.com/live/logo.png" rel="icon">
  <link href="https://liquag.com/live/logo.png" rel="apple-touch-icon">
...
<div class="card shadow-lg">
  <div class="card-body">
    <h5 class="card-title">Member File Uploads</h5>
    ...
    <hr style="margin-bottom: 1px">
                  ...
                  <div class="form-floating mb-3">
                    ...
                    <select name="member" class="form-select" id="member" aria-label="Member" required>
                      <option selected>Select Member</option>
                        ...
                        ?>
                        <option value="<?php echo $row['MemberNo']; ?>"><?php echo $row['MemberSurname']; ?> <?php echo $row['MemberFirstname']; ?></option>
                        <?php } ?>
                ...
                <div class="col-md-4">
                    <label for="delivery">Reason for Upload</label>
                  <div class="form-floating mb-3">
                    <select name="reason" class="form-select" id="reason" aria-label="Loan" required>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td> 
                    <a href='uploads/".$row['member']."/".$row['name']."' target='_blank' class='bi bi-eye-fill' style='font-size: 25px;' padding-right: 40px;></a>
                    &nbsp;&nbsp;
                   " ?>
                   <button type="button" data-link="dedit.php?id=<?php echo $row['id']; ?>" class="btn btn delete" data-id="<?php echo $row['id']; ?>" 
                   data-filepath="<?php echo "https://fairlife.grinpath.com/membership/uploads/".$row['member']."/".$row['name']." "?>">
