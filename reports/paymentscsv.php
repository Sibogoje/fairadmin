...
if($query->num_rows > 0){ 
    ...
    // Set column headers 
    $fields = array('MemberNo', 'Name', 'Surname', 'Transactiondate',  'Details',  'Amount'); 
    fputcsv($f, $fields, $delimiter); 
     
    // Output each row of the data, format line as csv and write to file pointer 
    while($row = $query->fetch_assoc()){ 

        $lineData = array($row['MemberNo'], $row['Name'], $row['Surname'], $row['Transactiodate'],  $row['Details'], $row['Amount']); 
