...
<div class="card col-lg-12" style="">
            <div class="card-body">
			  ...
			  <form class="row g-3 needs-validation" id="user_form" method="post" action="empcsv.php"  enctype="multipart/form-data" novalidate>
...
$stmt12 = $conn->prepare("SELECT DISTINCT `employerID`, `EmployerName` FROM `tblemployers` ");
						$stmt12->execute();
						$result12 = $stmt12->get_result();
						if ($result12->num_rows > 0) {
						  // output data of each row
						while($row12 = $result12->fetch_assoc()) {
						$fundname = $row12['EmployerName'];
						$retirementfund = $row12['employerID'];
...
echo $row12['EmployerName'] ; ?></option>
						<?php
if (!defined('APP_URL')) {
    $appConfigCandidates = array(
        __DIR__ . '/scripts/app_config.php',
    ...
    foreach ($appConfigCandidates as $appConfigPath) {
        ...
        <br/>
                  ...
                  <button type="submit"  class="btn btn-warning add" id="" data-link="" data-id="rr"  style="width: 100%;"><b>Download Employer Statement</b></button>
               </div>
   ...
   <div class="card col-lg-12" style="">
            <div class="card-body">
              <h5 class="card-title">Employer Statement</h5>
