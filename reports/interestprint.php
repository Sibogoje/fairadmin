...
<head>
    ...
    <title><?php
require_once __DIR__ . '/../scripts/bootstrap.php';
echo $ii;?> Interest Payout Print</title></title>
...
if (in_array("all", $mntharray)){
 // echo $mntharray2;
  //console("HHHH");
   $choose = "`interest` WHERE DATE(TransactionDate) BETWEEN '$d1'  AND '$d2'  ORDER BY TransactionDate DESC";  
}else{
  
 $choose = "`interest` WHERE `MemberNo` IN ({$mntharray2}) AND DATE(TransactionDate) BETWEEN '$d1'  AND '$d2'  ORDER BY TransactionDate DESC";   
}
		...
		<table class="table datatable responsive no-wrap"  id="free" style="margin-top: 20px; width: 100%; text-align: left;">
			<thead>
			    ...
			    <TR style="border: 0px;">
			        ...
			        <H2><span style="text-align: left;">Interest Paid</span> </H2>
			    </td>
...
$count++;
			$AMOUNT = $AMOUNT + $row12['Amount'];
						...
						?>
						 ...
						 <h3 style="text-align: right;">Total Interest Paid: E <?php
require_once __DIR__ . '/../scripts/bootstrap.php';
