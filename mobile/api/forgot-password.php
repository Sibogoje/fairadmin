...
$stmt = $pdo->prepare("SELECT answer1_hash, answer2_hash, answer3_hash FROM mobile_security_questions WHERE memberid = :memberno LIMIT 1");
$stmt->bindValue(':memberno', $memberNo, PDO::PARAM_STR);
$stmt->execute();
...
if (!password_verify(clean_answer($answers[0] ?? ''), $security['answer1_hash'])
    || !password_verify(clean_answer($answers[1] ?? ''), $security['answer2_hash'])
    || !password_verify(clean_answer($answers[2] ?? ''), $security['answer3_hash'])) {
